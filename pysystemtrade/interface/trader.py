...
class IBTrader(IBConnection, pub_sub.Subscriber):
    ...
    def place_limit_order(self, contract, type_, price, quantity, sleep=True):
        ...
        """
        # Generating order.
        order = ib.LimitOrder(type_, quantity, price, account=self.account)
    ...
    def place_market_order(self, contract, type_, quantity, sleep=True):
        """
        ...
        """
        # Generating order.
        order = ib.MarketOrder(type_, quantity, account=self.account)
